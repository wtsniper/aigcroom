...
function ToolLogo({ logoUrl, name, category }: { logoUrl: string; name: string; category: string }) {
  const [broken, setBroken] = useState(false)
  if (!logoUrl || broken) {
    return <span className="text-xl">{CATEGORY_ICON[category] || '🤖'}</span>
  }
  return (
    <img
      src={logoUrl}
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 bg-gray-800 rounded-xl flex items-center justify-center overflow-hidden shrink-0 ring-1 ring-white/10 group-hover:scale-110 transition-transform duration-300">
                      <ToolLogo logoUrl={tool.logoUrl} name={tool.name} category={tool.category} />
                    </div>
